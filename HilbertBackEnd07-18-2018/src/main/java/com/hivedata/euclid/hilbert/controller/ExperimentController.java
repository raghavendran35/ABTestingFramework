...
public class ExperimentController {
    ...
    public static String changeStatus(String expID, String newStatus,
        ...
        currExperiment.setLastEditedBy("" + (date.getMonth()+1)%12+ "-" +date.getDate() + "-" +(date.getYear()+1900));
        repository.save(currExperiment);
        return convertObject(currExperiment);
        }

    public static String changeDescription(String expID, String newDescription,
                                             String userID, ExpRepo repository){
        Experiment currExperiment = findByID(expID, repository);
        repository.delete(UUID.fromString(expID));
        currExperiment.setDescription(newDescription);
        currExperiment.setLastEditedBy("" + (date.getMonth()+1)%12+ "-" +date.getDate() + "-" +(date.getYear()+1900));
        repository.save(currExperiment);
        return convertObject(currExperiment);
        }
    ...
    public static String changeEndDate(String expID, String newEndDate, String userID, ExpRepo repository){
        Experiment currExperiment = findByID(expID, repository);
        repository.delete(UUID.fromString(expID));
        if (!dates.validateDates(currExperiment.getStartDate(), newEndDate)){
...
//change name of experiment
    public static String changeName(String expID, String newName,
                                      String userID, ExpRepo repository){
        Experiment currExperiment = findByID(expID, repository);
        repository.delete(UUID.fromString(expID));
        currExperiment.setName(newName);
        currExperiment.setLastEditedBy("" + (date.getMonth() + 1)%12+ "-" +date.getDate() + "-" +(date.getYear()+1900));
        repository.save(currExperiment);
        return convertObject(currExperiment);
    }
